...
(function () {
  ...
  function openAttractionModal(attraction) {
     // Сохраняем позицию скролла (из utils.js, если доступна, или реализуем здесь)
     const scrollY = window.scrollY || window.pageYOffset;
     document.body.setAttribute('data-scroll-position', scrollY);
     document.body.classList.add('modal-open');
     document.body.style.setProperty('--scrollbar-width', window.getScrollbarWidth() + 'px'); // Функция из utils.js
    ...
    if (priceElement) priceElement.textContent = window.formatPrice(attraction.price);
    if (mainImageElement) {
    ...
    if (specsContainer && attraction.specs) {
        ...
        for (const [key, value] of Object.entries(attraction.specs)) {
            // Можно добавить перевод ключей на русский
            let label = key;
    ...
    if (thumbnailsContainer) {
        ...
        thumb.className = 'thumbnail active'; // Предполагаем, что у вас есть .thumbnail в CSS
        thumb.addEventListener('click', () => {
             if(mainImageElement) {
                 mainImageElement.src = attraction.image;
                 // Убираем active у всех, добавляем этой
    ...
    if (buyNowBtn) {
        ...
        buyNowBtn.onclick = () => {
            ...
            if (cartBtn) cartBtn.click(); // Или вызвать openCartModal() из state.js, если она экспортирована
            // Если openCartModal не экспортирована, можно временно скопировать логику сюда или сделать экспорт
            // Для простоты, кликаем по кнопке корзины
        };
    }
  ...
  function setupEventListeners() {
    ...
    if (cartBtn) {
        // Предполагаем, что openCartModal доступна глобально или импортирована
        // Если нет, нужно добавить экспорт в state.js
        cartBtn.addEventListener('click', () => {
             // Сохраняем позицию скролла
             const scrollY = window.scrollY || window.pageYOffset;
             document.body.setAttribute('data-scroll-position', scrollY);
             document.body.classList.add('modal-open');
             document.body.style.setProperty('--scrollbar-width', window.getScrollbarWidth() + 'px');
             document.body.style.top = `-${scrollY}px`;

             // Открываем модалку корзины (логика из ui.js/main.js)
             // Так как мы не дублируем её, вызываем существующую функцию
             // Нужно убедиться, что она доступна глобально
             if (window.openCartModal) {
                 window.openCartModal(); // Вызываем из глобальной области
             } else {
                 console.warn('openCartModal function not found globally');
                 if(cartModal) {
                     cartModal.classList.add('open');
                     cartModal.setAttribute('aria-hidden', 'false');
                 }
                 // ... (минимальная логика открытия, если openCartModal недоступна)
             }
        });
    }
  ...
  // --- Exports (if needed) ---
  // Можно не экспортировать ничего, так как это самодостаточный модуль для страницы
